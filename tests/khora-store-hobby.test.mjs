...
test("12. el checkout inserta el flag de fabricación con el tipo de la base y evita doble envío", async () => {
  ...
  assert.match(route, /email=CASE WHEN NULLIF\(TRIM\(email\),\x27\x27\) IS NULL AND \?::text IS NOT NULL/);
  assert.match(route, /address=CASE WHEN NULLIF\(TRIM\(address\),\x27\x27\) IS NULL AND \?::text IS NOT NULL/);
