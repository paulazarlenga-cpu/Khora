...
import { AromaFinder } from "../khora-aroma-finder";
import { hasPublicSensoryContent, type PublicSensoryOption, type PublicSensoryProfile } from "../khora-aroma-match";
import { SensoryProfileDisplay } from "../khora-sensory-display";

type View = "home" | "product" | "cart" | "details" | "confirmation" | "aroma";
type StoreCollection = { id: number; name: string; slug: string; description: string; sortOrder: number };
type Product = { id: number; code: string; name: string; description: string; category: string; type: string; priceCents: number; stock: number; availableStock: number; imagePath: string | null; published: boolean; unitsSold?: number; collectionMemberships: Array<{ collectionId: number; sortOrder: number }>; sensoryProfile: PublicSensoryProfile };
...
const emptyPublicSensoryProfile: PublicSensoryProfile = { families: [], notes: [], sensations: [], intensity: null, rooms: [], moment: null };
...
const viewFromLocation = (): { view: View; productId?: number; orderNumber?: string; orderAccess?: string } => {
  ...
  if (params.get("vista") === "aroma") return { view: "aroma" };
...
export default function StorePage() {
  ...
  const [sensoryOptions, setSensoryOptions] = useState<PublicSensoryOption[]>([]);
  ...
  const selectedCollection = useMemo(() => collections.find((collection) => collection.slug === collectionSlug), [collections, collectionSlug]);
  const filteredProducts = useMemo(() => products.filter((product) => (!selectedCollection || product.collectionMemberships.some((membership) => membership.collectionId === selectedCollection.id)) && `${product.name} ${product.category} ${product.description}`.toLowerCase().includes(query.toLowerCase().trim())).sort((left,right) => {
    if (!selectedCollection) return left.name.localeCompare(right.name,"es");
    const leftOrder=left.collectionMemberships.find((membership)=>membership.collectionId===selectedCollection.id)?.sortOrder??Number.MAX_SAFE_INTEGER;
    const rightOrder=right.collectionMemberships.find((membership)=>membership.collectionId===selectedCollection.id)?.sortOrder??Number.MAX_SAFE_INTEGER;
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setLoading(true);
    fetch(`/api/tienda?entity=products${token ? `&token=${encodeURIComponent(token)}` : ""}`).then(async (response) => { const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "No pudimos cargar la colección. Revisá tu conexión e intentá nuevamente."); return data as { products: Product[]; collections: StoreCollection[]; sensoryOptions?: PublicSensoryOption[]; reservationExpiresAt?: string }; }).then((data) => { if (active) { setProducts(data.products); setCollections(data.collections ?? []); setSensoryOptions(data.sensoryOptions ?? []); if (data.reservationExpiresAt) { setExpiresAt(data.reservationExpiresAt); try { localStorage.setItem("khora-store-expires", data.reservationExpiresAt); } catch { /* optional persistence */ } } setError(""); } }).catch((cause) => { if (active) setError(friendlyError(cause, "No pudimos cargar la colección. Revisá tu conexión e intentá nuevamente.")); }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [token, catalogRefresh]);

  useEffect(() => {
    const refreshWhenVisible = () => { if (document.visibilityState === "visible") setCatalogRefresh((value) => value + 1); };
  ...
  function navigate(nextView: View, productId?: number, orderNumber?: string, orderAccess?: string) {
    ...
    if (nextView === "aroma") params.set("vista", "aroma");
  ...
  async function syncReservation(nextCart: CartLine[]) {
    if (!nextCart.length) { if (token) await fetch("/api/tienda", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "release", token }) }).catch(() => undefined); saveCart([]); setNotice(""); setExpiresAt(""); setReservationExpired(false); try { localStorage.removeItem("khora-store-expires"); localStorage.removeItem("khora-store-token"); } catch { /* optional persistence */ } return; }
  ...
  async function addToCart(product: Product, quantity = 1) {
    ...
    try { await syncReservation(next); setNotice(`${product.name} quedó reservado en tu carrito.`); return true; } catch { /* message already visible */ return false; }
  }
  ...
  return <div className={styles.storeShell}>
    <StoreHeader cartCount={cartCount} filled={cart.length > 0} query={query} onHome={() => navigate("home")} onCatalog={() => { navigate("home"); setTimeout(() => document.getElementById("catalogo")?.scrollIntoView({ behavior: "smooth" }), 20); }} onStory={() => { navigate("home"); setTimeout(() => document.getElementById("historia")?.scrollIntoView({ behavior: "smooth" }), 20); }} onAroma={() => navigate("aroma")} onCart={() => navigate("cart")} onQueryChange={(value) => { setQuery(value); if (view !== "home") navigate("home"); }} />
    ...
    {view === "home" && <>
      <main><section className={styles.hero} aria-labelledby="store-hero-title"><div className={styles.heroCopy}><div className={styles.eyebrowRow}><p className={styles.eyebrow}>OBJETOS PARA HABITAR DESPACIO</p><span aria-hidden="true" /></div><h1 id="store-hero-title">Lo<br />cotidiano,<br /><em>con<br />intención.</em></h1><p>Pequeños objetos hechos para acompañar tu casa y tus momentos de todos los días.</p><button className={`${styles.primary} ${styles.discoverButton}`} onClick={() => document.getElementById("catalogo")?.scrollIntoView({ behavior: "smooth" })}><span className={styles.discoverSweep} aria-hidden="true" /><span className={styles.discoverText}>Descubrir KHORA</span><span className={styles.discoverArrow} aria-hidden="true">→</span></button><button className={styles.heroAromaCta} onClick={() => navigate("aroma")}>ENCONTRÁ TU AROMA <span>→</span></button><div className={styles.heroMeta}><span>01 <i aria-hidden="true" /> 03</span><span>SCROLLÉA <b aria-hidden="true">↓</b></span></div></div><div className={styles.heroImage}><img src="/khora-store-hero.png" alt="Difusor de aroma sobre piedra y cerámica, rodeado de hojas" /><span className={styles.heroImageNote}>AROMAS · OBJETOS · HOGAR</span></div></section><BestSellersSection products={bestSellerProducts} onOpen={(product) => navigate("product", product.id)} /><section className={styles.catalog} id="catalogo"><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>LA COLECCIÓN</p><h2>Elegidos para tu espacio</h2></div><p>Diseño simple, materiales nobles y una pausa para lo esencial.</p></div><div className={styles.filters}><div className={styles.categoryList}><button className={collectionSlug === "all" || !selectedCollection ? styles.selectedFilter : ""} onClick={() => setCollectionSlug("all")}>Todas</button>{collections.map((collection) => <button key={collection.id} className={collectionSlug === collection.slug ? styles.selectedFilter : ""} onClick={() => setCollectionSlug(collection.slug)}>{collection.name}</button>)}</div><label className={styles.catalogSearch}><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por nombre…" aria-label="Buscar en el catálogo" /></label></div>{loading ? <p className={styles.empty}>Cargando la colección…</p> : error && !products.length ? <div className={styles.emptyPanel} role="alert"><h2>No pudimos cargar la colección</h2><p>Revisá tu conexión e intentá nuevamente.</p><button className={styles.primary} onClick={() => { setError(""); setCatalogRefresh((value) => value + 1); }}>Reintentar <span>→</span></button></div> : <div className={styles.grid}>{filteredProducts.map((product) => <ProductCard key={product.id} product={product} onOpen={() => navigate("product", product.id)} onAdd={() => addToCart(product)} disabled={saving} />)}</div>}{!loading && !error && !filteredProducts.length && <p className={styles.empty}>{selectedCollection ? `No hay productos disponibles en ${selectedCollection.name}.` : "No encontramos productos con esa búsqueda."}</p>}</section><section className={styles.story} id="historia"><div><p className={styles.eyebrow}>LA MIRADA KHORA</p><h2>Hecho para quedarse.</h2></div><p>Creamos objetos honestos, calmos y duraderos. Cada pieza encuentra su lugar cuando suma belleza sin pedir atención.</p><button className={styles.linkButton} onClick={() => document.getElementById("catalogo")?.scrollIntoView({ behavior: "smooth" })}>Ver la colección <span>→</span></button></section><section className={styles.editorialBanner} aria-label="Manifiesto KHORA"><div><p className={styles.eyebrow}>UNA PAUSA EN CASA</p><h2>Una casa también<br />se recuerda<br /><em>por su aroma.</em></h2></div><p>Objetos sencillos para rituales que se vuelven parte de vos.</p></section><section className={styles.newsletter} aria-label="Novedades de KHORA"><div><p className={styles.eyebrow}>DE VEZ EN CUANDO</p><h2>Un poco de KHORA.</h2></div><div><p>Novedades, objetos y pequeñas historias para habitar despacio.</p><div className={styles.newsletterField}><input type="email" placeholder="Tu correo electrónico" aria-label="Tu correo electrónico" disabled /><button type="button" disabled aria-label="Suscribirse próximamente">→</button></div><small>Próximamente.</small></div></section></main><Footer /></>}
    {view === "aroma" && <AromaFinder products={products} options={sensoryOptions} saving={saving} onOpenProduct={(id) => navigate("product", id)} onAddProduct={async (id) => { const product = products.find((item) => item.id === id); return product ? addToCart(product) : false; }} />}
...
function StoreHeader({ cartCount, filled, query, onHome, onCatalog, onStory, onAroma, onCart, onQueryChange }: StoreHeaderProps) {
  return <header className={`${styles.header} ${styles.publicHeader}`}>
    ...
    <nav aria-label="Store navigation">
      ...
      <button className={styles.navItem} onClick={onStory}>Nosotros</button><button className={styles.navItem} onClick={onAroma}>Encontrá tu aroma</button>
    </nav>
    <div className={styles.headerActions}>
      ...
      <button className={`${styles.textAction} ${styles.bagAction}`} onClick={onCart} aria-label="Abrir bolsa"><span className={styles.bagAnimationFrame}><span className={styles.bagIconWrap}><svg className={`${styles.bagIcon} ${filled ? styles.bagIconFilled : ""}`} viewBox="0 0 24 24" aria-hidden="true"><path d="M5.5 8.5h13l1 11h-15l1-11Z" fill={filled ? "currentColor" : "none"} stroke="currentColor" strokeWidth="1.5" /><path d="M9 9V6.5a3 3 0 0 1 6 0V9" fill="none" stroke="currentColor" strokeWidth="1.5" /></svg></span><span className={styles.bagLabel}>Bolsa {cartCount ? `(${cartCount})` : ""}</span></span></button>
    </div>
  </header>;
}
...
type AddButtonPhase = "idle" | "adding" | "added";
type AddToCartButtonProps = { variant: "link" | "solid"; onAdd: () => Promise<boolean>; disabled?: boolean };

function AddToCartButton({ variant, onAdd, disabled = false }: AddToCartButtonProps) {
...
function ProductDetail({ product, onBack, onAdd, disabled = false }: { product: Product; onBack: () => void; onAdd: (quantity: number) => Promise<boolean>; disabled?: boolean }) {
  const [quantity, setQuantity] = useState(1);
  const unavailable = product.availableStock <= 0;
  const sensoryProfile = product.sensoryProfile ?? emptyPublicSensoryProfile;
  const hasSensoryProfile = hasPublicSensoryContent(sensoryProfile);
  return <main className={styles.detailPage}><button className={styles.backLink} onClick={onBack}><span className={styles.backLinkArrow} aria-hidden="true">←</span><span>Volver a la colección</span></button><div className={styles.detailGrid}><div className={styles.detailVisual}><ProductImage product={product} alt={product.name} /></div><div className={styles.detailInfo}><p className={styles.eyebrow}>{product.category}</p><h1>{product.name}</h1><p className={styles.detailPrice}>{money(product.priceCents)}</p><div className={styles.availability}>{unavailable ? "Sin stock" : product.availableStock <= 3 ? `Últimas ${formatQuantity(product.availableStock)} unidades` : "Disponible"}</div>{!unavailable && <div className={styles.quantityRow}><div className={styles.quantity}><button onClick={() => setQuantity((value) => Math.max(1, value - 1))} aria-label="Disminuir cantidad">−</button><span>{quantity}</span><button onClick={() => setQuantity((value) => Math.min(product.availableStock, value + 1))} aria-label="Aumentar cantidad">+</button></div><AddToCartButton variant="solid" onAdd={() => onAdd(quantity)} disabled={disabled} /></div>}<div className={styles.accordion}><p>Materiales y cuidado</p><span>{product.description || "La información disponible de cada producto se actualiza desde KHORA Administración."}</span></div>{hasSensoryProfile && <SensoryProfileDisplay profile={sensoryProfile} description="" />}</div></div></main>;
}
...
function EmptyCartArtwork() {
  return <div className={styles.cartEmptyArtwork} aria-hidden="true"><svg viewBox="0 0 152 112" role="presentation"><ellipse className={styles.cartEmptyHalo} cx="66" cy="51" rx="50" ry="38" /><path className={styles.cartEmptyBag} d="M37 39h54l5 54H32l5-54Z" /><path className={styles.cartEmptyHandle} d="M49 40V27c0-11 7-18 15-18s15 7 15 18v13" /><path className={styles.cartEmptyBranch} d="M97 92c13-16 19-32 20-54" /><path className={styles.cartEmptyLeaf} d="M105 77c8 0 14-4 18-11-8-1-14 2-18 11Zm7-15c8-2 13-7 15-14-8 1-13 5-15 14Zm3-16c6-3 10-8 11-14-7 2-10 6-11 14Z" /></svg></div>;
}

function CartEmptyDivider() {
  return <div className={styles.cartEmptyDivider} aria-hidden="true"><span /><svg viewBox="0 0 22 26"><path d="M5 24C8 16 13 9 20 3M8 16c-4 0-6-2-7-5 4 0 6 1 7 5Zm5-7c-4 0-6-2-7-5 4 0 6 1 7 5Zm4-6c-3 0-4-1-5-3 3 0 4 1 5 3Z" /></svg><span /></div>;
}
