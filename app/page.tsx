...
const descriptions: Partial<Record<SectionId, string>> = {
  inicio: "Así está el negocio hoy.", ventas: "Registrá cobros y seguí cada venta sin perder el stock.", clientes: "Conocé a tus clientes y detectá a quién volver a contactar.", productos: "Precios, costos, recetas y combos en un solo lugar.", fabricacion: "Planificá lotes y consumí materias primas automáticamente.", stock: "Controlá productos terminados y materias primas en tiempo real.", compras: "Registrá abastecimiento, comprobantes y pagos a proveedores.", proveedores: "Contactos, insumos y evolución de precios de cada proveedor.", finanzas: "Entendé cuánto vende y cuánto gana realmente el negocio.", calendario: "Organizá fabricación, compras y cobros del negocio.",
};
...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${mobileNav ? "is-open" : ""}`} aria-label="Navegación principal">
      ...
      <nav className="side-nav"><span className="nav-caption">GESTIÓN</span>{navigation.map((item) => <button key={item.id} className={section === item.id ? "active" : ""} onClick={() => { setMobileNav(false); goTo(item.id); }}><i aria-hidden="true"><KhoraIcon name={item.icon} /></i><span>{item.label}</span>{item.id === "stock" && stockSeverity !== "normal" && <b className={`nav-alert ${stockSeverity}`} title={stockAlertDescription} aria-label={`Stock ${stockSeverity === "out" ? "sin stock" : "con poco stock"}: ${stockAlertDescription}`}>!</b>}</button>)}</nav>
