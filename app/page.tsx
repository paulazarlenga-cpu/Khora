...
type AdminDashboardSnapshot = DashboardSnapshot & { batches: Array<Record<string, unknown>>; suppliers: Array<Record<string, unknown>>; mixtures: Array<Record<string, unknown>>; orders: Array<Record<string, unknown>>; orderAttention: number; dismissedAlertIds: string[] };
const emptyDashboardSnapshot: AdminDashboardSnapshot = { summary: {}, products: [], materials: [], sales: [], clients: [], purchases: [], profitability: [], profits: [], batches: [], suppliers: [], mixtures: [], orders: [], orderAttention: 0, dismissedAlertIds: [] };
export default function Home() {
    ...
    fetch("/api/khora?entity=dashboard_snapshot")
      ...
      .then((data) => {
        ...
        const snapshot: AdminDashboardSnapshot = {
          ...
          batches: rows(data.batches), suppliers: rows(data.suppliers), mixtures: rows(data.mixtures), orders: rows(data.orders), orderAttention: Number(data.orderAttention) || 0, dismissedAlertIds: Array.isArray(data.dismissedAlertIds) ? data.dismissedAlertIds.filter((id): id is string => typeof id === "string") : [],
        };
        setDashboardSnapshot(snapshot);
        setOperationalData({ clients: snapshot.clients, products: snapshot.products, materials: snapshot.materials, batches: snapshot.batches, suppliers: snapshot.suppliers, purchases: snapshot.purchases, mixtures: snapshot.mixtures, orders: snapshot.orders });
  ...
  return <div className="app-shell">
    <header className="desktop-navbar">
      <button className="navbar-brand" onClick={() => goTo("inicio")} aria-label="Ir al inicio de KHORA"><KhoraLogo variant="wordmark" size="md" theme="white" decorative /></button>
      ...
      <div className="navbar-user"><a className="navbar-store-link" href="/tienda" target="_blank" rel="noreferrer"><span>Ver Tienda</span><span aria-hidden="true">↗</span></a><div className="profile-wrap"><button className="navbar-profile" onClick={() => setProfileOpen((value) => !value)} aria-expanded={profileOpen}><span className="avatar">PZ</span><span><strong>Paula</strong><small>Administradora</small></span><i><KhoraIcon name="chevron-down" /></i></button>{profileOpen && <div className="profile-menu"><button onClick={() => { setProfileOpen(false); setProfilePanel("settings"); }}><KhoraIcon name={moduleIcons.configuracion} /> Configuración</button><button onClick={() => { setProfileOpen(false); setProfilePanel("profile"); }}><KhoraIcon name="users" /> Mi perfil</button><button onClick={signOut}>Cerrar sesión</button></div>}</div></div>
    </header>
    <aside className={`sidebar ${mobileNav ? "is-open" : ""}`} aria-label="Navegación principal">
      ...
      <a className="sidebar-store-link" href="/tienda" target="_blank" rel="noreferrer" onClick={() => setMobileNav(false)}><span>Ver Tienda</span><span aria-hidden="true">↗</span></a>
