...
function Customers({ search }: { search: string }) {
  ...
  const filtered = insights.filter((insight) => includesSearch(insight.customer, search));
  const recovery = getRecoveryCustomers();
  const [tab, setTab] = useState("Clientes");
  const [selected, setSelected] = useState<NonNullable<CustomerInsight> | null>(null);
  const [whatsappCustomer, setWhatsappCustomer] = useState<NonNullable<CustomerInsight> | null>(null);
  return <div className="section-stack"><Tabs tabs={["Clientes", `Para recuperar · ${recovery.length}`]} active={tab} onChange={setTab} /><div className="summary-row three"><MiniStat label="Clientes frecuentes" value={String(insights.filter((item) => item.label === "Frecuente").length)} detail="Compran de forma sostenida" tone="success" /><MiniStat label="Mayoristas" value={String(insights.filter((item) => item.label === "Mayorista").length)} detail="Lista asignada automáticamente" tone="info" /><MiniStat label="Para recuperar" value={String(recovery.length)} detail="Superaron su frecuencia habitual" tone="danger" /></div>{tab === "Clientes" ? <><Toolbar placeholder="Buscar por nombre, teléfono o localidad…" filters={["Todos los clientes", "Clasificación", "Lista de precios"]} /><Panel title="Clientes" subtitle={`${filtered.length} clientes visibles`}><DataTable headers={["Cliente", "Contacto", "Última compra", "Pedidos", "Ticket promedio", "Deuda", "Clasificación", ""]}>{filtered.map((insight) => <tr key={insight.customer.id}><td><CellPerson name={insight.customer.name} subtitle={insight.customer.location} initialsText={insight.customer.initials} /></td><td><span>{insight.customer.phone}</span><small className="table-sub">{insight.customer.email}</small></td><td>{insight.customer.last}</td><td>{insight.customer.orders}</td><td><strong>{money(insight.averageTicket)}</strong></td><td className={insight.pendingDebt ? "danger-text" : ""}>{insight.pendingDebt ? money(insight.pendingDebt) : "—"}</td><td><Badge tone={insight.tone}>{insight.label}</Badge><small className="table-sub">{insight.priceList.name}</small></td><td><button className="table-open-button" onClick={() => setSelected(insight)}>Ver ficha</button></td></tr>)}</DataTable></Panel></> : <RecoveryCustomers items={recovery} onOpen={setSelected} onWhatsApp={setWhatsappCustomer} />}{selected && <CustomerDetail insight={selected} onClose={() => setSelected(null)} onWhatsApp={() => setWhatsappCustomer(selected)} />}{whatsappCustomer && <WhatsAppComposer customer={whatsappCustomer.customer} defaultTemplate="recovery" onClose={() => setWhatsappCustomer(null)} />}</div>;
}

function RecoveryCustomers({ items, onOpen, onWhatsApp }: { items: Array<NonNullable<CustomerInsight>>; onOpen: (item: NonNullable<CustomerInsight>) => void; onWhatsApp: (item: NonNullable<CustomerInsight>) => void }) {
  if (!items.length) return <div className="empty-recovery"><span>✓</span><h2>Todos los clientes están dentro de su frecuencia habitual</h2><p>No hay sugerencias de recuperación por ahora.</p></div>;
  return <div className="recovery-grid">{items.map((insight) => <article key={insight.customer.id}><header><Avatar text={insight.customer.initials} /><div><h2>{insight.customer.name}</h2><p>{insight.customer.location}</p></div><Badge tone="danger">Inactivo</Badge></header><div className="recovery-delay"><strong>{insight.daysWithoutBuying} días</strong><span>desde la última compra</span><i><em style={{ width: `${Math.min(100, insight.daysWithoutBuying / insight.recoveryThresholdDays * 100)}%` }} /></i><small>Frecuencia habitual: cada {insight.customer.usualFrequencyDays} días</small></div><p className="recovery-reason">Lleva {insight.daysWithoutBuying - insight.customer.usualFrequencyDays} días más de lo habitual sin comprar.</p><footer><button className="secondary-button" onClick={() => onOpen(insight)}>Ver ficha</button><button className="primary-button" onClick={() => onWhatsApp(insight)}>Preparar WhatsApp</button></footer></article>)}</div>;
}

function CustomerDetail({ insight, onClose, onWhatsApp }: { insight: NonNullable<CustomerInsight>; onClose: () => void; onWhatsApp: () => void }) {
...
function WhatsAppComposer({ customer, order, defaultTemplate = "confirm", onClose }: { customer: (typeof customers)[number]; order?: OrderRecord; defaultTemplate?: WhatsAppTemplate; onClose: () => void }) {
  ...
  return <div className="whatsapp-layer"><button className="drawer-backdrop" onClick={onClose} aria-label="Cerrar mensaje" /><section className="whatsapp-composer" role="dialog" aria-modal="true" aria-labelledby="whatsapp-title"><header><div><p>MENSAJE PREPARADO</p><h2 id="whatsapp-title">WhatsApp para {customer.name}</h2><span>{customer.phone}</span></div><button onClick={onClose} aria-label="Cerrar">×</button></header><div><label><span>Motivo</span><select value={template} onChange={(event) => changeTemplate(event.target.value as WhatsAppTemplate)}>{available.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><label><span>Revisá el mensaje antes de continuar</span><textarea rows={11} value={message} onChange={(event) => setMessage(event.target.value)} /></label><div className="whatsapp-safe-note"><i>i</i><p>KHORA no envía mensajes automáticamente. Se abrirá WhatsApp con este borrador para que lo revises nuevamente.</p></div></div><footer><button className="secondary-button" onClick={onClose}>Cancelar</button><a className="primary-button" href={buildWhatsAppLink(customer.phone, message)} target="_blank" rel="noreferrer">Abrir WhatsApp ↗</a></footer></section></div>;
}
...
function Suppliers({ search }: { search: string }) {
  const rows = suppliers.filter((supplier) => includesSearch(supplier, search));
  return <div className="section-stack"><div className="summary-row three"><MiniStat label="Proveedores activos" value="12" detail="4 categorías de insumos" tone="info" /><MiniStat label="Comprado este año" value={money(2860400)} detail="42 compras" tone="neutral" /><MiniStat label="Saldo pendiente" value={money(118400)} detail="1 proveedor" tone="warning" /></div><Toolbar placeholder="Buscar proveedor, contacto o insumo…" filters={["Todos los rubros", "Última compra"]} /><div className="supplier-grid">{rows.map((supplier) => <article className="supplier-card" key={supplier.name}><header><Avatar text={supplier.initials} /><div><h3>{supplier.name}</h3><p>{supplier.supplies}</p></div><MoreButton /></header><dl><div><dt>Contacto</dt><dd>{supplier.contact}</dd></div><div><dt>WhatsApp</dt><dd>{supplier.phone}</dd></div><div><dt>Última compra</dt><dd>{supplier.lastPurchase}</dd></div><div><dt>Total comprado</dt><dd>{money(supplier.total)}</dd></div></dl><footer><button>Ver ficha completa</button><button className="whatsapp-link">Contactar ◉</button></footer></article>)}</div></div>;
}
