...
type ProductRow = { id: number; code: string; name: string; type: string; combo_id?: number; sale_price_cents: number; current_stock: number; minimum_stock: number; estimated_cost_cents: number; last_batch_unit_cost_cents: number; profit_percentage: number; active: boolean } & Record<string, unknown>;
...
type ComboRow = { id: number; product_id: number; code: string; combo: string; components: number; current_stock: number; estimated_cost_cents: number; last_batch_unit_cost_cents: number; active: boolean; updated_at: string } & Record<string, unknown>;
...
function Products({ search, onCreateMaterial }: { search: string; onCreateMaterial: () => void }) {
  ...
  const filtered = productData.rows.filter((product) => includesSearch(product, search));
  const active = productData.rows.filter((product) => Boolean(product.active));
  ...
  function completed(message: string) { setShowProductForm(false); setShowComboForm(false); refreshAll(); setProductNotice(message); window.setTimeout(() => setProductNotice(""), 4200); }
  ...
  return <div className="section-stack">
    ...
    {(tab === "Productos" || tab === "Recetas") && <div className="product-create-row"><div><strong>Producto y receta son definiciones</strong><span>Crearlos no suma stock ni consume materias primas.</span></div><button className="primary-button" onClick={() => setShowProductForm(true)}>＋ Nuevo producto</button></div>}
    {tab === "Productos" && <><div className="summary-row"><MiniStat label="Productos activos" value={String(active.length)} detail={`${categoryData.rows.filter((category) => category.kind === "PRODUCT" && category.active).length} categorías`} tone="info" /><MiniStat label="Valor del stock" value={money(stockValueCents / 100)} detail="A costo actual" tone="neutral" /><MiniStat label="Stock bajo" value={String(lowStock.length)} detail="Requieren fabricación" tone="warning" /><MiniStat label="Margen promedio" value={`${averageMargin.toFixed(1)}%`} detail="Sobre precio de venta" tone="success" /></div><Toolbar placeholder="Buscar producto o código…" filters={["Todos los tipos", "Todos los estados"]} /><div className="product-grid">{filtered.map((product) => { const cost = productCost(product), margin = productMargin(product), isLow = Number(product.current_stock) <= Number(product.minimum_stock), tone: Tone = isLow ? "danger" : "success"; return <article className="product-card" key={product.id}><div className="product-visual"><div className="product-photo-empty"><span>{product.type === "COMBO" ? "K" : "kh"}</span><small>Imagen privada próximamente</small></div><div className="product-visual-actions"><Badge tone={tone}>{isLow ? "Stock bajo" : "Disponible"}</Badge></div></div><div className="product-body"><small>{product.code} · {productTypeLabel(product.type)}</small><div className="product-title-row"><h3>{product.name}</h3><span className={`stock-chip ${tone}`}>Stock {formatQuantity(Number(product.current_stock))} u.</span></div><div className="product-numbers"><div><span>Precio</span><strong>{money(Number(product.sale_price_cents) / 100)}</strong></div><div><span>Costo</span><strong>{money(cost / 100)}</strong></div><div><span>Margen</span><strong>{margin.toFixed(1)}%</strong></div></div><footer><span>{product.active ? "Producto activo" : "Producto inactivo"}</span><button onClick={() => setSelectedProduct(product)}>Ver detalle →</button></footer></div></article>; })}</div>{!productData.loading && !filtered.length && <div className="recipe-empty">Todavía no hay productos registrados.</div>}</>}
    {tab === "Recetas" && <RecipeList rows={recipeData.rows} loading={recipeData.loading} onCreate={() => setShowProductForm(true)} />}
    {tab === "Combos" && <ComboList rows={comboData.rows} products={productData.rows} loading={comboData.loading} onCreate={() => setShowComboForm(true)} />}
    {tab === "Categorías" && <SimpleCategories rows={categoryData.rows} />}
    {selectedProduct && <ProductProfitabilityDetail product={selectedProduct} profitability={profitabilityData.rows.find((row) => Number(row.id) === Number(selectedProduct.id))} onClose={() => setSelectedProduct(null)} />}
...
function ProductProfitabilityDetail({ product, profitability, onClose }: { product: ProductRow; profitability?: ProfitabilityRow; onClose: () => void }) {
...
function RecipeList({ rows, loading, onCreate }: { rows: RecipeRow[]; loading: boolean; onCreate: () => void }) { return <Panel title="Recetas de fabricación" subtitle={loading ? "Actualizando…" : `${rows.length} recetas registradas`} action={<button className="primary-button" onClick={onCreate}>＋ Nueva receta</button>}><DataTable headers={["Producto", "Rendimiento", "Ingredientes", "Costo receta", "Costo unitario", "Actualizada", "Estado"]}>{rows.map((recipe) => { const yieldQuantity = Number(recipe.yield_quantity) || 1; const unitCost = Number(recipe.estimated_cost_cents); return <tr key={recipe.id}><td><strong>{recipe.product}</strong><small className="table-sub">{recipe.code}</small></td><td>{formatQuantity(yieldQuantity)} u.</td><td>{Number(recipe.components)} componentes</td><td>{money(unitCost * yieldQuantity / 100)}</td><td><strong>{money(unitCost / 100)}</strong></td><td>{formatDate(recipe.updated_at)}</td><td><Badge tone={recipe.active ? "success" : "neutral"}>{recipe.active ? "Activa" : "Inactiva"}</Badge></td></tr>; })}</DataTable>{!loading && !rows.length && <div className="recipe-empty">Todavía no hay recetas registradas.</div>}</Panel>; }
function ComboList({ rows, products: productRows, loading, onCreate }: { rows: ComboRow[]; products: ProductRow[]; loading: boolean; onCreate: () => void }) { return <Panel title="Combos" subtitle={loading ? "Actualizando…" : `${rows.length} combos registrados`} action={<button className="primary-button" onClick={onCreate}>＋ Nuevo combo</button>}><div className="combo-grid">{rows.map((combo) => { const product = productRows.find((candidate) => Number(candidate.id) === Number(combo.product_id)); const cost = Number(combo.last_batch_unit_cost_cents) > 0 ? Number(combo.last_batch_unit_cost_cents) : Number(combo.estimated_cost_cents); return <article key={combo.id}><div className="combo-icon">K</div><div><small>{combo.code}</small><h3>{combo.combo}</h3><p>{Number(combo.components)} componentes · stock {formatQuantity(Number(combo.current_stock))} u.</p><dl><div><dt>Costo</dt><dd>{money(cost / 100)}</dd></div><div><dt>Precio</dt><dd>{money(Number(product?.sale_price_cents ?? 0) / 100)}</dd></div></dl><Badge tone={combo.active ? "success" : "neutral"}>{combo.active ? "Activo" : "Inactivo"}</Badge></div></article>; })}</div>{!loading && !rows.length && <div className="recipe-empty">Todavía no hay combos registrados.</div>}</Panel>; }
